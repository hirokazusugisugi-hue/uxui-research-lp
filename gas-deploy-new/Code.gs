...
// ★ 以下を自分の情報に変更してください
// ====================================
var ADMIN_EMAIL = 'uxuiosaka@gmail.com,hirokazusugisugi@gmail.com,stevenm.kawasaki@gmail.com'; // 通知を受け取るメールアドレス
...
function setupSheets() {
  ...
  Logger.log('セットアップ完了: 見学申込・お問い合わせシートを作成しました。');
}
...
function doGet(e) {
  ...
  if (action === 'setup') {
    setupSheets();
    return ContentService
      .createTextOutput(JSON.stringify({ result: 'ok', message: 'セットアップ完了。シートを作成しました。' }))
      .setMimeType(ContentService.MimeType.JSON);
  }

  return ContentService
    .createTextOutput(JSON.stringify({ result: 'ok', message: 'UX/UI研究会フォームAPI 稼働中' }))
    .setMimeType(ContentService.MimeType.JSON);
}
